...
const TokenCard = ({ token, onRotate, onRevoke }: TokenCardProps) => {
	...
	return (
		<div
			className={
				'flex flex-col gap-3 rounded-[12px] bg-default p-4 w-full'
			}
		>
			<div className={'flex items-center justify-between'}>
				...
				<div className={'flex items-center gap-1 shrink-0'}>
					...
					<Popover>
						<Popover.Trigger>
							<Button variant={'tertiary'} size={'sm'} color={'danger'}>
								<span
									className={
										'material-icons-round !text-[18px] !leading-none'
									}
						...
						<Popover.Content>
							<div
								className={
									'flex flex-col gap-3 p-4 max-w-[260px]'
								}
							>
			...
			<div className={'flex flex-wrap gap-1'}>
				{token.scopes.map((scope) => (
					<Chip key={scope} size={'sm'} variant={'outline'}>
						{scope}
					</Chip>
				))}
			</div>
